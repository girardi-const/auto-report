...
import { Loader2, UploadCloud, FileText, FileSpreadsheet, Trash2, AlertTriangle, ExternalLink, Activity, Clock, Info, X, Copy, CheckCheck, Ban } from 'lucide-react';
...
export default function ImportPage() {
    ...
    const FORMAT_PROMPT = `Formate essa planilha Excel (.xlsx ou .csv) com os seguintes campos obrigatórios e opcionais:
...
- Link de Imagem      [OPCIONAL]    — URL direta da imagem do produto (informe para vincular imagem ao produto)
Regras:
...
`;
    ...
    const parseFilePreview = (file: File) => {
        ...
        reader.onload = (evt) => {
            try {
                ...
                if (json.length === 0) { setPreviewData(null); return; }
                const headers = (json[0] as string[]).map(String);
                const rows = json.slice(1, 11).map((r) => headers.map((_, i) => String((r as string[])[i] ?? '')));
    ...
    const handleFileChange = (e: ChangeEvent<HTMLInputElement>) => {
        ...
        // Reset the input so the same file can be re-selected after clear
        e.target.value = '';
    };
    ...
    return (
        <div className="min-h-screen bg-muted">
            ...
            <div className="max-w-5xl mx-auto px-4 sm:px-6 pb-12">
                                                ...
                                                <table className="w-full text-xs">
                                                    ...
                                                    <tbody className="divide-y divide-gray-50">
                                                        {[
                                                            ...
                                                            { col: 'Link de Imagem', req: false, detail: 'URL direta da imagem (opcional). Se informada, a imagem será baixada e vinculada ao produto. Se omitida, o produto ficará sem imagem.' },
                                                        ].map(({ col, req, detail }) => (
              ...
              ${selectedFile ? 'border-green-400 bg-green-50/60' : 'border-gray-300 hover:border-primary hover:bg-red-50/30 bg-gray-50'}`}
                                ...
                                >
                                    ...
                                    {selectedFile ? (
                                        ...
                                        <div className="flex items-center gap-4 p-5">
                                            {/* File type icon */}
                                            <div className={`flex-shrink-0 w-14 h-14 rounded-xl flex items-center justify-center shadow-sm ${selectedFile.name.endsWith('.xlsx') ? 'bg-green-100' : 'bg-blue-100'
                                                }`}>
                                            ...
                                            <div className="flex-1 min-w-0">
                                                ...
                                                <div className="flex items-center gap-2 mt-1">
                                                    <span className={`inline-block px-2 py-0.5 rounded-md text-[10px] font-black uppercase tracking-wide ${selectedFile.name.endsWith('.xlsx')
                                                ...
                                                <p className="text-[10px] text-green-600 font-semibold mt-1.5">✓ Arquivo pronto para importação</p>
                                            </div>
                                    ...
                                    <div className="rounded-xl border border-gray-100 overflow-hidden">
                                        <div className="flex items-center justify-between px-4 py-2.5 bg-gray-50 border-b border-gray-100">
                                            <span className="text-xs font-bold text-gray-500 uppercase tracking-widest">
                                                Pré-visualização — primeiras {previewData.rows.length} linhas
                                            </span>
                                            <span className="text-[10px] text-gray-400 font-medium">{previewData.headers.length} colunas detectadas</span>
                                        </div>
                                                ...
                                                <thead className="sticky top-0 bg-white z-10">
                                                    <tr>
                                                        {previewData.headers.map((h, i) => (
                                                            <th
                                                                key={i}
                                                                className="px-3 py-2 text-left font-bold text-gray-600 border-b border-gray-100 whitespace-nowrap bg-gray-50/80"
                                                            >
                                                                {h || <span className="text-gray-300 italic">sem nome</span>}
                                                            </th>
                                                        ))}
                                                    </tr>
                                                </thead>
                                                    ...
                                                    {previewData.rows.map((row, ri) => (
                                                        ...
                                                        >
                                                            {row.map((cell, ci) => (
                                                                <td
                                                                    key={ci}
                                                                    className="px-3 py-2 text-gray-700 border-b border-gray-50 whitespace-nowrap max-w-[200px] truncate"
                                                                    title={cell}
                                                                >
