...
function buildSectionsPayload(
  sections: Section[],
  calculateSubtotal: (prods: Section["products"], disc: number) => number,
) {
...
export default function ReportForm({
      ...
      timers.current[productId] = setTimeout(async () => {
        ...
        try {
          const res = await fetch(
            `${process.env.NEXT_PUBLIC_API_URL || "http://localhost:3001/api/v1"}/products/${code}`,
          );
          if (res.ok) {
  ...
  const validSections = sections
    .map((s) => {
      const validProducts = s.products.filter(
        (p) =>
          p.code &&
          p.code.trim() !== "" &&
          p.name &&
      ...
      const groupedProducts = validProducts.reduce(
        (acc, current) => {
  ...
  const subtotalBeforeCash = validSections.reduce(
    (acc, s) => acc + calculateSubtotal(s.products, s.discount),
    0,
  );
  const totalValue =
    subtotalBeforeCash * (1 - (cashDiscount || 0) / 100) + (deliveryFee || 0);
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8 flex flex-col gap-8 animate-in fade-in duration-700">
      ...
      <GeneralInfo
        data={{ especificador, consultor, consultorPhone }}
        onChange={{ setEspecificador, setConsultor, setConsultorPhone }}
      />
      ...
      <div className="flex flex-col gap-8">
        {sections.map((section) => (
          <SectionCard
            ...
            actions={{
              ...
              batchAddProducts,
              updateSectionMargin,
              updateSectionDiscount: (id, discount) =>
                setSections(
                  sections.map((s) => (s.id === id ? { ...s, discount } : s)),
                ),
            }}
        ...
        >
          ...
          <div className="flex flex-col items-center gap-1">
            ...
            <span className="text-[10px] font-medium text-gray-400 uppercase tracking-widest group-hover:text-primary/50 transition-colors">
              Agrupe seus produtos por ambiente
            </span>
          </div>
        </button>
      </div>
            ...
            <div className="flex flex-col sm:flex-row items-center gap-2 sm:gap-4">
              <button
                onClick={() => {
                  ...
                  const cleanTitle =
                    reportTitle &&
                    reportTitle.toLowerCase().includes("orçamento")
                      ? reportTitle.replace(/orçamento/i, "").trim()
                      : (reportTitle || clientInfo.name).replace(/\s+/g, "_");
                  const date = new Date()
