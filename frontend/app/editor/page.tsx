        ...
        if (items[i].type.indexOf("image") !== -1) {
          ...
          reader.onload = (event) => {
            ...
            if (activeEl && activeEl.hasAttribute("data-cell-id")) {
              const [blockId, rIdxStr, cIdxStr] = activeEl
    ...
    try {
      ...
      if (download) {
        ...
        const date = new Date()
          ...
          .replace(/\//g, "-"); // Replaces / with - for file system compatibility

        // 2. Clean the title to prevent "Orçamento_Orçamento"
        const cleanTitle =
          doc.title && doc.title.toLowerCase().includes("orçamento")
            ? doc.title.replace(/orçamento/i, "").trim()
            : (doc.title || doc.clientName).replace(/\s+/g, "_");
        link.download = `Orçamento_${cleanTitle}_${date}.pdf`.replace(
          /__+/g,
          "_",
        );
        link.click();
          ...
          <div className="bg-white p-4 rounded-2xl shadow-sm border border-gray-200">
            ...
            <div className="flex flex-col gap-2">
              <button
                ...
                className="flex items-center gap-3 px-3 py-2.5 bg-gray-50 border border-transparent hover:border-blue-200 hover:bg-blue-50 hover:text-blue-700 rounded-xl text-xs font-semibold text-gray-600 transition-all group"
              >
                <Type
                  size={14}
              ...
              <button
                ...
                className="flex items-center gap-3 px-3 py-2.5 bg-gray-50 border border-transparent hover:border-purple-200 hover:bg-purple-50 hover:text-purple-700 rounded-xl text-xs font-semibold text-gray-600 transition-all group"
              >
                <TableIcon
                  size={14}
              ...
              <button
                ...
                className="flex items-center gap-3 px-3 py-2.5 bg-gray-50 border border-transparent hover:border-green-200 hover:bg-green-50 hover:text-green-700 rounded-xl text-xs font-semibold text-gray-600 transition-all group"
              >
                <Heading
                  size={14}
              ...
              <button
                onClick={() => addBlock("section_total")}
                className="flex items-center gap-3 px-3 py-2.5 bg-gray-50 border border-transparent hover:border-gray-300 hover:bg-gray-200 hover:text-gray-800 rounded-xl text-xs font-semibold text-gray-600 transition-all group"
              >
                <Sigma
                  size={14}
              ...
              <button
                onClick={() => addBlock("general_total")}
                className="flex items-center gap-3 px-3 py-2.5 bg-gray-50 border border-transparent hover:border-blue-300 hover:bg-blue-100 hover:text-blue-800 rounded-xl text-xs font-semibold text-gray-600 transition-all group"
              >
                <Receipt
                  size={14}
                  className="text-blue-500 group-hover:scale-110 transition-transform"
              ...
              <button
                ...
                className="flex items-center gap-3 px-3 py-2.5 bg-gray-50 border border-transparent hover:border-orange-200 hover:bg-orange-50 hover:text-orange-700 rounded-xl text-xs font-semibold text-gray-600 transition-all group"
              >
                <ImageIcon
                  size={14}
              ...
              <button
                ...
                className="flex items-center gap-3 px-3 py-2.5 bg-gray-50 border border-transparent hover:border-gray-300 hover:bg-gray-100 hover:text-gray-900 rounded-xl text-xs font-semibold text-gray-600 transition-all group"
              >
                <AlignLeft
                  size={14}
                  className="text-gray-500 group-hover:scale-110 transition-transform"
        ...
        <div className="w-full max-w-[800px] flex-shrink-0 min-h-[1131px] bg-white shadow-2xl relative px-10 pt-8 pb-24 font-sans text-xs border border-gray-200">
          ...
          <div className="bg-[#f2f2f2] px-2 py-1.5 mb-2 mt-2 group relative">
            <input
              value={doc.title}
              onChange={(e) =>
                setDoc({ ...doc, title: e.target.value.toUpperCase() })
              }
          ...
          <div className="space-y-1">
            ...
            {doc.blocks.map((block, i) => (
              <div
                key={block.id}
                className="group relative transition-all border border-transparent hover:border-blue-100 hover:bg-blue-50/20 -mx-4 px-4 py-1"
              >
                <div className="absolute -left-8 top-1/2 -translate-y-1/2 flex flex-col gap-1 opacity-0 group-hover:opacity-100 transition-opacity z-10">
                  <button
                    ...
                    className="p-1 bg-white border border-gray-200 rounded-full text-gray-400 hover:text-black shadow-sm"
                  >
                    <MoveUp size={12} />
                  </button>
                  <button
                  ...
                  <button
                    ...
                    className="p-1.5 mt-1 bg-red-50 text-red-500 rounded-full hover:bg-red-100 shadow-sm"
                  >
                    <Trash2 size={12} />
                  </button>
                </div>
                ...
                <div className="w-full relative">
                  ...
                  {block.type === "text" && (
                    <div className="space-y-3">
                      ...
                      <textarea
                        ...
                        className={`w-full bg-gray-50 border border-gray-200 rounded-md px-3 py-2 outline-none resize-y min-h-[100px] ${block.style === "h1"
                            ? "text-2xl font-bold"
                            : block.style === "h2"
                              ? "text-lg font-bold"
                              : block.style === "small"
                  ...
                  {block.type === "image" && (
                    <div className="space-y-3">
                      ...
                      <div
                        className={`flex w-full ${block.align === "center" ? "justify-center" : block.align === "right" ? "justify-end" : "justify-start"}`}
                      >
                        {block.src ? (
                          ...
                          <div className="w-full h-32 bg-gray-100 flex items-center justify-center text-gray-400 text-sm border-2 border-dashed border-gray-300 rounded">
                            Dê CTRL+V aqui ou ao vivo na página para colar a
                            imagem
                          </div>
                        )}
                      </div>
                        ...
                        <table className="w-full text-[10px] text-left font-sans border-t-[0.5px] border-black border-b-[0.5px]">
                          <thead className="bg-[#f2f2f2] text-black font-bold uppercase">
                            <tr>
                              {block.headers.map((h, hidx) => (
                                <th
                                  key={hidx}
                                  className="px-1 py-1.5 border-r border-transparent hover:border-gray-300 last:border-r-0"
                                  style={{
                                    width: block.columnWidths
                                      ? `${block.columnWidths[hidx]}%`
                                      : "auto",
                                    textAlign: "center",
                                  }}
                                >
                          ...
                          <tbody className="divide-y divide-[#f2f2f2]">
                            {block.rows.map((r, ridx) => (
                              <tr
                                key={ridx}
                                className="group/row hover:bg-[#f9f9f9]"
                              >
                                {r.map((c, cidx) => {
                                  const isImage =
                                  ...
                                  return (
                                    <td
                                      key={cidx}
                                      className="px-1 py-2 text-center align-middle relative"
                                      style={{
                                        width: block.columnWidths
                                          ? `${block.columnWidths[cidx]}%`
                                          : "auto",
                                      }}
                                    >
                                      {isImage ? (
                      ...
                      {(block as GeneralTotalBlock).items.map((item, idx) => (
                        ...
                        >
                          <input
                            ...
                            onChange={(e) => {
                              const newItems = [
                                ...(block as GeneralTotalBlock).items,
                              ];
          ...
          <div className="bg-[#efede7] py-2 flex flex-col items-center justify-center mt-6 w-full -mx-4 px-4 shadow-sm border border-gray-200/50">
            <span className="text-[10px] font-bold text-[#333333] mb-1 text-center">
              *condição de pagamento à prazo em até 10x sem juros no cartão
            </span>
            <span className="text-[10px] font-bold text-[#333333] text-center">
          ...
          <div className="absolute bottom-6 left-10 right-10 flex justify-between items-end text-[10px] font-bold border-t-[0.5px] border-gray-400 pt-1">
            ...
            <div className="w-[30%] uppercase text-right">
              Consultor de Vendas
            </div>
          </div>
        </div>
      ...
      <div className="fixed bottom-6 left-1/2 -translate-x-1/2 flex items-center gap-3 bg-white p-2 rounded-2xl shadow-xl border border-gray-200 z-40">
        ...
        >
          {generating ? (
            ...
            <>
              <FileDown size={20} /> Pré-Visualizar
            </>
          )}
        </button>
        <button
